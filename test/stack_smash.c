...
{
        key_value_t data_param[2]; 
        char buff[5000];
        insert_key_value(data_param, "offset", "730076240"); 
	insert_key_value(data_param, "limit", "1");

        char *urlparam = http_build_query(data_param, sizeof(data_param) / sizeof(data_param[0]));
        sprintf(buff, "https://api.telegram.org/bot%s/%s?%s", "1666834345:AAHRfz_efeQq3lgNrx_MOWUAR8Epps12UpA", "getUpdates", urlparam);
        printf("%s", buff);
        return aa;
}
